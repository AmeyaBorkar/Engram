...
def _load_suite_module():
    ...
    spec = importlib.util.spec_from_file_location(
        "_longmemeval_test_module", _SUITE_PATH
    )
...
def test_v3_template_file_exists_and_has_required_slots() -> None:
    ...
    template = (_ROOT / "benchmarks" / "suites" / "prompts" / "longmemeval_answer_v3.txt").read_text(
        encoding="utf-8"
    )
...
def test_cli_parser_rejects_unknown_prompt_version() -> None:
    ...
    with pytest.raises(SystemExit):
        parser.parse_args(
            ["run", "longmemeval", "--chat", "fake", "--embedder", "fake", "--prompt-version", "v99"]
        )
